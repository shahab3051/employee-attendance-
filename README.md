...
**Users**
`Employee id | Password | Role | EMP ID`
- `Role` is `Admin` or `Employee`. Anyone who should see every employee's
  data (HR included) just gets `Role = Admin` — there's no separate "HR"
  role, Admin already means "see everything."
- `Password` here is stored as **plain text** and compared directly —
  matches how the current sheet is set up. If you'd rather not store
  plain-text passwords, rename the column to `PasswordHash` and put a
  SHA-256 hex digest in it instead (generate one by temporarily running
  `hashPassword("thePassword")` in the Apps Script editor's console) —
  `login()` checks for either column name automatically.
- The login field itself is `Employee id` (e.g. `abdulsaboor#5`) — the
  code also accepts a column literally named `Username` if you ever
...
**Employee mapping is automatic** — no list to maintain in the script.
The bridge fetches the Employee tab's EMP ID + Name straight from your
Sheet (on startup, and again every `DIRECTORY_REFRESH_EVERY` polls) and
matches each machine's numeric user ID directly against EMP ID.
- Enroll every employee on **both** machines using their **EMP ID**
  (e.g. `5`, `441`, `452`...) as the device's numeric user ID.
- Add a new employee to the Employee tab and they're picked up
  automatically on the next refresh — nothing to edit in the script.
- Any device user ID that isn't a real EMP ID (old test enrollments,
  other staff, etc.) is skipped quietly — you'll just see a one-line
  summary in the log instead of a warning per scan.

**Nothing gets missed on a fresh start.** Each poll checks the last
`LOOKBACK_HOURS` (default 36h — "yesterday and today") of each
machine's attendance log, not just the last minute or two. So the
first time you run this, it backfills whatever's already sitting on
the machines automatically. Re-sending a scan that was already pushed
is harmless — `Code.gs` ignores duplicates for a slot that's already
filled.
...
2. In `zkteco_bridge.py`, fill in:
   ...
   - `APPS_SCRIPT_URL`, `BRIDGE_SECRET` (same value as the Script Property)
