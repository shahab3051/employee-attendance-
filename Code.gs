 ...
 *
 *  Users
 *    Employee id | Password | Role (Admin/Employee) | EMP ID
 *    (Also accepts Username/PasswordHash if you use those labels instead —
 *    login() checks both. Password is compared as plain text unless the
 *    column is literally named PasswordHash, in which case it's compared
 *    as a SHA-256 hash. HR staff who should see every employee's data
 *    just get Role = Admin, same as any other admin account — there's no
 *    separate "HR" role to configure.)
 *
 *  Requests
 ...
 *
 *    The bridge no longer needs a hand-maintained device-user-id map —
 *    it fetches the Employee tab's EMP ID + Name straight from this
 *    Sheet (getEmployeeDirectoryForBridge, BRIDGE_SECRET-authenticated)
 *    and matches each machine's numeric user ID against EMP ID directly.
 *    Enroll each employee on both machines using their EMP ID as the
 *    device's numeric user ID, and it just works — add a new employee
 *    to the Employee tab and they're picked up automatically.
 * =============================================================
 */
...
function doPost(e) {
  try {
    ...
    // Login, and the two ZKTeco bridge endpoints, don't require an existing session —
    // the bridge authenticates with BRIDGE_SECRET instead, since it runs unattended.
    ...
    if (action === 'getEmployeeDirectory') return jsonOut({ success: true, ...getEmployeeDirectoryForBridge(body) });
...
/** Lets the ZKTeco bridge fetch the current Employee list directly from
 *  the Sheet (EMP ID + Name only) so it can match each machine's device
 *  user ID against real employees automatically — no manual mapping to
 *  maintain in the Python script. Authenticated with BRIDGE_SECRET. */
function getEmployeeDirectoryForBridge(body) {
  if (body.secret !== getSecret('BRIDGE_SECRET')) throw new Error('Invalid bridge secret.');
  const employees = readSheet(SHEET_NAMES.EMPLOYEES).map(normalizeEmployee);
  return { employees: employees.map(e => ({ employeeId: e.employeeId, name: e.name })) };
}
...
/** Looks up a field on a Users-sheet row by one or more possible column
 *  labels (case/spacing-insensitive) — the real sheet uses "Employee id"
 *  / "Password" rather than "Username" / "PasswordHash", so this keeps
 *  login working whichever labels are actually in the sheet. */
function findUserField(row, ...labelAliases) {
  const wanted = labelAliases.map(normalizeHeader);
...
function login(username, password) {
  ...
  const user = users.find(u => {
    const uname = findUserField(u, 'Username', 'Employee id', 'Employee ID', 'Login ID', 'User ID');
    return String(uname || '').toLowerCase() === String(username).toLowerCase();
  });
  if (!user) throw new Error('Invalid username or password.');

  const storedHash = findUserField(user, 'PasswordHash', 'Password Hash');
  const storedPlain = findUserField(user, 'Password');
  const passwordOk = storedHash !== undefined
    ? hashPassword(password) === storedHash
    : String(password) === String(storedPlain);
  if (!passwordOk) throw new Error('Invalid username or password.');

  const usernameVal = findUserField(user, 'Username', 'Employee id', 'Employee ID', 'Login ID', 'User ID');
  const roleVal = findUserField(user, 'Role');
  const role = String(roleVal || '').toLowerCase() === 'admin' ? 'admin' : 'employee';
  ...
  if (!employee) employee = { id: userEmpId || '', name: usernameVal, department: '' };

  const token = makeToken({ username: usernameVal, role, employeeId: employee.id, exp: Date.now() + TOKEN_TTL_MS });
  return { token, role, employee: { id: employee.id, username: usernameVal, name: employee.name, department: employee.department } };
}
